...
## 🌐 Language Bindings (Separate Repositories - HIGH PRIORITY)

Native language bindings are **essential for library adoption**. These are maintained in separate repositories but are a strategic priority. See `docs/BINDINGS_IMPLEMENTATION_GUIDE.md` for implementation details.

### Core Repository Task: C FFI Foundation

#### Task FFI-001: Add C FFI Layer to Core Library
**ID:** `FFI-001`  
**Priority:** Critical  
**Complexity:** Medium  
**Estimated Time:** 1 week  
...
**Description:**
Add a C-compatible FFI layer to the core redstr library. This enables all language bindings (Node.js, Python, .NET, etc.).

**Requirements:**
- Add `ffi` feature flag to Cargo.toml
- Create `src/ffi.rs` with C-compatible exports
- Add `cbindgen.toml` for header generation
- Export all public functions via C FFI
- Handle string allocation/deallocation safely
- Add `cdylib` and `staticlib` crate types

**Deliverables:**
```
src/ffi.rs (new)
include/redstr.h (generated by cbindgen)
cbindgen.toml (new)
Cargo.toml (updated with ffi feature)
```

**Acceptance Criteria:**
- [ ] FFI layer compiles with `--features ffi`
- [ ] C header generated successfully
- [ ] Memory safety verified (no leaks)
- [ ] All public functions exported
- [ ] Tests verify FFI functionality
- [ ] Documentation explains FFI usage

**Dependencies:** None

---

### Binding Repositories (Separate Repos)

| Repository | Framework | Package Manager | Priority | Status |
|------------|-----------|-----------------|----------|--------|
| `redstr-npm` | napi-rs | npm | 🔴 Critical | Planned |
| `redstr-wasm` | wasm-bindgen | npm | 🔴 Critical | Planned |
| `redstr-python` | PyO3 + maturin | PyPI | 🟡 High | Planned |
| `redstr-csharp` | C FFI + P/Invoke | NuGet | 🟡 High | Planned |
| `redstr-go` | CGO | Go modules | ✅ Exists | Active |

### Why Separate Repositories?

Based on industry best practices (tree-sitter, swc, Polars, libsodium):
- **Independent versioning** - Bindings can release independently
- **Language-specific CI/CD** - Each needs different build systems
- **Clear ownership** - Different maintainers for different ecosystems
- **Focused codebases** - Smaller, more manageable repos
